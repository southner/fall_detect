...
def add_mask(pic,tag):
    ...
    for j in range(13):
        if tag[0,j,2]>tag[0,j,5]:
            ...
            index = 5
        if (tag[0,j,index]>0.8):
            if (tag[0,j,6]>tag[0,j,7]):
                one_mask = cv2.rectangle(mask,(0,j*5),(pic.shape[1],j*5+4),color=(0,255,0),thickness=-1)
            else:
                one_mask = cv2.rectangle(mask,(0,j*5),(pic.shape[1],j*5+4),color=(255,0,0),thickness=-1)
            mask+=one_mask
...
if __name__ == '__main__':
    for i, (rd,ra,re,tag,pic) in enumerate(train_data_loader):
        visualize_fall(rd[0],ra[0],re[0],pic[0],tag,tag)
        pass