...
class MyLoss(nn.Module):
    ...
    def compute_iou(self, box1, box2):
        ...
        lt = torch.max(
            # [N,1] -> [N,1,1] -> [N,M,1]
            box1[:, :1].unsqueeze(1).expand(N, M, 1),
            # [M,1] -> [1,M,1] -> [N,M,1]
            box2[:, :1].unsqueeze(0).expand(N, M, 1),
        )
        # 右下角选择小的
        # 距离远点选择小的
        rb = torch.min(
    ...
    def forward(self, pred_tensor, target_tensor):
        ...
        coo_fall_mask = (target_tensor[:, :, 2] > self.threld) & (
            target_tensor[:, :, 7] == 1)
        coo_normal_mask = (target_tensor[:, :, 2] > self.threld) & (
            target_tensor[:, :, 6] == 1)

        coo_mask = coo_mask.unsqueeze(-1).expand_as(target_tensor)
        noo_mask = noo_mask.unsqueeze(-1).expand_as(target_tensor)
        coo_fall_mask = coo_fall_mask.unsqueeze(-1).expand_as(target_tensor)
        coo_normal_mask = coo_normal_mask.unsqueeze(
            -1).expand_as(target_tensor)
        ...
        nooobj_loss = F.mse_loss(
            F.relu(noo_pred_c-self.threld+0.3), noo_target_c, size_average=False)

        coo_response_mask = torch.cuda.ByteTensor(
            box_target.size())  # [batch_size*13*2,3]
        coo_response_mask.zero_()
        coo_not_response_mask = torch.cuda.ByteTensor(box_target.size())
        coo_not_response_mask.zero_()

        # 选择最好的IOU 2个box选1个吧
        # 2是有每个距离bin有两个预测框，i是他们的起始index，之后根据iou选择mask
        for i in range(0, box_target.size()[0], 2):
            # 预测框 2个
            # box_pred是预测框 shape为(一个batch中target存在的bin数目*2,3)
            box1 = box_pred[i:i+2]
            ...
            #  标注框 1个
            # box_target是目标框 shape为(一个batch中target存在的bin数目,3)
            box2 = box_target[i].view(-1, 3)
        ...
        contain_loss = F.mse_loss(
            box_pred_response[:, 2], box_target_response[:, 2], size_average=False)
        #temp_loc_loss = F.mse_loss(torch.sqrt(box_pred_response[:,1]),torch.sqrt(box_target_response[:,1]),size_average=False)
        loc_loss = F.mse_loss(
            box_pred_response[:, 0], box_target_response[:, 0], size_average=False)
        ...
        not_contain_loss = F.mse_loss(
            box_pred_not_response[:, 2], box_target_not_response[:, 2], size_average=False)+1e-6
        ...
        class_fall_loss = self.cross_entropy(
            class_pred_fall, class_target_fall) if len(class_pred_fall) > 0 else 0
        class_normal_loss = self.cross_entropy(
            class_pred_normal+1e-0006, class_target_normal)
        ...
        # nooobj_loss 本来无预测有的置信度loss
        # class_loss 类别loss
        total_loss = (self.loc_scale*loc_loss + self.cotain_scale*contain_loss +
                      not_contain_loss + self.l_noobj*nooobj_loss + self.class_scale*class_loss)+1e-6
...
class CountIndex(nn.Module):
    ...
    def forward(self, pred_tensor, target_tensor):
        ...
        # 选择confidence较大的box
        detect_pred_choose = torch.max(
            pred_tensor[:, :, 2], pred_tensor[:, :, 5])
        # pred 中是否存在目标
        detect_pred_mask = detect_pred_choose > self.threld

        DTT = torch.sum((detect_tar_mask == 1) & (
            detect_pred_mask == 1), dim=[0, 1])
        DTF = torch.sum((detect_tar_mask == 1) & (
            detect_pred_mask == 0), dim=[0, 1])
        DFT = torch.sum((detect_tar_mask == 0) & (
            detect_pred_mask == 1), dim=[0, 1])
        DFF = torch.sum((detect_tar_mask == 0) & (
            detect_pred_mask == 0), dim=[0, 1])
        ...
        classsify_pred = torch.where(
            classsify_pred[:, 0] > classsify_pred[:, 1], 1, 0)
        classsify_targ = torch.where(
            classsify_targ[:, 0] > classsify_targ[:, 1], 1, 0)
...
class HeatLoss(nn.Module):
    ...
    def forward(self, predict, heatmap):
        ...
        return self.lambda_1*loss_sm + self.lambda_2*loss_jhm + self.lambda_3*loss_paf
