...
class RadDateset(Dataset):
    ...
    def __init__(self,
        ...
        for dir_name in train_pos:  # 非跌倒 label为0
            ...
            if (not opt.exists(opt.join(dir,'open_body'))):
                continue
            ...
            # 0-27 2-29 ... 414-441
            for frame_index in range(30, frame_num-30, 3):
                slice = [frame_index-1,frame_index,frame_index+1]
                sample = {}
                sample['RD'] = radar_data_doppler[slice, 1:, :]
                sample['RA'] = radar_data_azimuth[slice, 1:, :]
                sample['pic'] = opt.join(dir, 'color_res/{:0>5d}.jpg'.format(frame_index))
