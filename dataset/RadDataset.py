...
class RadDateset(Dataset):
    ...
    def __init__(self,
        ...
        self.window_step = 5
        self.window_size = 10
        experiments_config = pd.read_excel(
            config['dataset']['experiments_config_path'], keep_default_na=False)
        train_pos = []  # 非跌倒
        train_neg = []  # 跌倒
        for i in range(len(experiments_config['dir_name'].values)):
            if experiments_config['person'][i] in config['dataset']['train_config']['person'] and experiments_config['place'][i] in config['dataset']['train_config']['place'] and experiments_config['person_num'][i] in config['dataset']['train_config']['person_num']:
            ...
            for frame_index_begin in range(0, frame_num-30, 2):
                ...
                for person_id in range(0, 3):
                    ...
                    if (np.mean(person_range) >= config['dataset']['range_config']['range_max']):
                        continue
                    range_index = int(
                        np.mean(person_range)/config['dataset']['range_config']['range_res']//1)
                    is_useful = True
                    sample['Tag'][range_index, 0] = np.mean(
                        person_range)/config['dataset']['range_config']['range_res'] % 1
                    sample['Tag'][range_index, 1] = (
                        person_range[1]-person_range[0])/2/config['dataset']['range_config']['range_res']
                    sample['Tag'][range_index, 2] = 1
                    sample['Tag'][range_index, 3] = np.mean(
                        person_range)/config['dataset']['range_config']['range_res'] % 1
                    sample['Tag'][range_index, 4] = (
                        person_range[1]-person_range[0])/2/config['dataset']['range_config']['range_res']
        ...
        for dir_name in train_neg:  # 跌倒 label为1
            ...
            for fall_person_index in range(1, 4):
                ...
                for bias in range(-5, 8):  # 23-53
                    is_useful = False
                    slice = [i for i in range(
                        fall_index-15+bias, fall_index+15+bias, 3)]
                    # 0-39  20  5-32 0-27 12-40
                    sample = {}
                    sample['RD'] = radar_data_doppler[slice, 1:, :]
                    sample['RA'] = radar_data_azimuth[slice, 1:, :]
                    sample['RE'] = radar_data_elevation[slice, 1:, :]
                    # (mid,range,confidence,label)
                    sample['Tag'] = np.zeros(
                        [config['dataset']['range_config']['range_num'], 8])
                    sample['pic'] = opt.join(
                        dir, 'color_res/{:0>5d}.jpg'.format(fall_index+bias))
                    mid = fall_index+bias
